...
class SilentAudioStreamer(AudioStreamer):
    ...
    async def stop_streaming(self):
        ...
        # Note: stream_manager.close() is not called here to allow reuse in while True loop
        # The caller (run_text_only) will handle cleanup in its finally block
...
class ConversationInterrupted(Exception):
    """Custom exception to signal that conversation was interrupted by user."""
    pass
...
class TextInputHandler:
    ...
    async def start_text_conversation(self):
        ...
        try:
            ...
            try:
            ...
            except (KeyboardInterrupt, ConversationInterrupted):
                ...
                if self.input_task and not self.input_task.done():
                    ...
                    try:
                        ...
                        pass
                # Re-raise as KeyboardInterrupt to propagate to run_text_only's while loop
                raise KeyboardInterrupt("Conversation interrupted by user")
        ...
        while self.is_active:
            try:
                ...
                try:
                    ...
                    raise ConversationInterrupted("Input task cancelled")
                except KeyboardInterrupt as e:
                    # KeyboardInterrupt from input() - convert to ConversationInterrupted
                    raise ConversationInterrupted("Keyboard interrupt in input") from e
                except Exception as e:
                    # Check if the underlying exception is KeyboardInterrupt
                    if isinstance(e.__cause__, KeyboardInterrupt) or isinstance(e, KeyboardInterrupt):
                        raise ConversationInterrupted("Keyboard interrupt in input") from e
                    raise
